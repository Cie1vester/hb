...
/* ══════════════════════════════════════════════════════════════
   ART — drop in your own character images and they replace the
   drawn sprites entirely. Nothing else needs changing.

     assets/syl.png    → Syl
     assets/wife.png   → your wife

   Use PNGs with a TRANSPARENT background. Small pixel art is ideal:
   anything shorter than the target height is scaled up by a whole
   number with nearest-neighbour, so it stays crisp and blocky.
   Bigger images are smoothly scaled down instead.

   If a file isn't there, the drawn fallback is used, so the site
   never breaks while you're still finding art. `h` is the on-screen
   height in game pixels — raise or lower it to taste.
   ══════════════════════════════════════════════════════════════ */

const ART = {
  purin: { src: 'assets/syl.png', h: 52, img: null, ready: false },
  kuromi: { src: 'assets/wife.png', h: 58, img: null, ready: false }
};
...
for (const key in ART) {
  ...
  im.onerror = () => { a.ready = false; };   // no file → keep the drawn version
  im.src = a.src;
}

function drawArt(g, a, x, y, lift) {
  const im = a.img;
  const up = im.height <= a.h;
  const scale = up ? Math.max(1, Math.round(a.h / im.height)) : a.h / im.height;
  const dw = Math.round(im.width * scale), dh = Math.round(im.height * scale);
  g.imageSmoothingEnabled = !up;             // nearest-neighbour only when scaling up
  g.drawImage(im, Math.round(x) - Math.round(dw / 2), Math.round(y) - dh - Math.round(lift || 0), dw, dh);
...
function drawPurin(g, x, y, o = {}) {
  ...
  if (ART.purin.ready) { drawArt(g, ART.purin, x, y, lift + (o.bob || 0)); return; }
...
function drawKuromi(g, x, y, o = {}) {
  ...
  if (ART.kuromi.ready) { drawArt(g, ART.kuromi, x, y, o.bob || 0); return; }
